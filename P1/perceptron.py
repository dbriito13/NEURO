...
if __name__ == "__main__":
    ...
    while max_incremento_pesos > 0 and n_epoch < max_epochs:
        ...
        for i in range(0, len(entradas_datos)):
            ...
            t = salidas_datos[i][0]
            y = capa_salida.neuronas[0].valor_salida
            #print(capa_salida.neuronas[0].valor_entrada)
            if y != t:
                for neurona in capa_entrada.neuronas:
                    max_incremento_pesos = max(max_incremento_pesos, tasa*t*neurona.valor_salida)
                    neurona.conexiones[0].peso += tasa*t*neurona.valor_salida
